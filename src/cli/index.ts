...
import { buildInitFiles, getDefaultInitOptions, type InitOptions } from "./templates.js";
import {
  buildDryRunOutput,
  buildNextSteps,
...
async function uninstall(dryRun: boolean): Promise<void> {
  if (dryRun) {
    console.log(formatBanner());
    console.log("");
...
async function promptForOptions(): Promise<InitOptions> {
  ...
  try {
    ...
    const provider = await choose(
      ...
      ["openai", "anthropic"],
      "openai",
      "Choose the provider whose API key you will add to GitHub Actions secrets."
    );
    const trigger = await choose(
      rl,
      "When should PullGuard run?",
      ["always", "label", "comment"],
      "comment",
      "`label` means apply `run-pullguard`; `comment` means comment `/pullguard`."
    );
